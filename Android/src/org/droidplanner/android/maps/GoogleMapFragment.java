...
public class GoogleMapFragment extends SupportMapFragment implements DPMap {
    ...
    /**
     * Modern (async, non-blocking) location handling. The legacy
     * {@code LocationServices.FusedLocationApi} + {@code GoogleApiClient} path
     * blocked the main thread with a CountDownLatch on newer play-services and
     * caused ANRs / frozen touch. This uses {@link FusedLocationProviderClient}.
     */
    private FusedLocationProviderClient mFusedLocationClient;

    @android.annotation.SuppressLint("MissingPermission")
    private void startLocationUpdates() {
        if (mFusedLocationClient == null || !hasLocationPermission()) return;

        final LocationRequest locationReq = new LocationRequest.Builder(
                Priority.PRIORITY_HIGH_ACCURACY, USER_LOCATION_UPDATE_INTERVAL)
                .setMinUpdateIntervalMillis(USER_LOCATION_UPDATE_FASTEST_INTERVAL)
                .setMinUpdateDistanceMeters(USER_LOCATION_UPDATE_MIN_DISPLACEMENT)
                .build();

        try {
            mFusedLocationClient.requestLocationUpdates(locationReq, locationCb, handler.getLooper());
        } catch (SecurityException e) {
            Timber.w(e, "Missing location permission for updates.");
        }
    }

    private void stopLocationUpdates() {
        if (mFusedLocationClient != null) {
            mFusedLocationClient.removeLocationUpdates(locationCb);
        }
    }

    @android.annotation.SuppressLint("MissingPermission")
    private void requestLastLocation(final OnSuccessListener<Location> onSuccess) {
        if (mFusedLocationClient == null || !hasLocationPermission()) {
            ...
            mFusedLocationClient.getLastLocation()
                    ...
                    .addOnFailureListener(new com.google.android.gms.tasks.OnFailureListener() {
                        ...
                        public void onFailure(@NotNull Exception e) {
                            Timber.w(e, "getLastLocation failed");
                            onSuccess.onSuccess(null);
                        }
                    });
        } catch (SecurityException e) {
    ...
    public void setLocationListener(android.location.LocationListener receiver) {
        ...
        if (mLocationListener != null) {
            requestLastLocation(new OnSuccessListener<Location>() {
                @Override
