...
public class StatusBarNotificationProvider implements NotificationHandler.NotificationProvider {
    ...
    /**
     * Android status bar's notification id. Shared with {@link org.droidplanner.android.AppService}
     * so the foreground-service notification and the rich "connected" notification
     * are the same one.
     */
    public static final int NOTIFICATION_ID = 1;

    /** Notification channel for the ongoing flight/connection status notification. */
    public static final String CHANNEL_ID = "tower_flight_status";
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            if (nm != null && nm.getNotificationChannel(CHANNEL_ID) == null) {
                final NotificationChannel channel = new NotificationChannel(CHANNEL_ID,
                        context.getString(R.string.app_title), NotificationManager.IMPORTANCE_LOW);
    ...
    StatusBarNotificationProvider(Context context, Drone api) {
        ...
        mNotificationIntent = PendingIntent.getActivity(mContext, 0, new Intent(mContext,
                FlightActivity.class), PendingIntent.FLAG_IMMUTABLE);

        mToggleConnectionIntent = PendingIntent
                .getBroadcast(mContext, 0, new Intent(DroidPlannerApp.ACTION_TOGGLE_DRONE_CONNECTION)
                        .setPackage(mContext.getPackageName()), PendingIntent.FLAG_IMMUTABLE);
    }
