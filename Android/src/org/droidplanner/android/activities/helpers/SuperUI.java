...
public abstract class SuperUI extends AppCompatActivity implements DroidPlannerApp.ApiListener,
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mAppServiceBound = bindService(new Intent(context, AppService.class), this,
                Context.BIND_AUTO_CREATE);
    }

    private boolean mAppServiceBound;
    ...
    public void onDestroy() {
        ...
        if (mAppServiceBound) {
            try {
                ...
                // Already unbound / never registered.
            }
            mAppServiceBound = false;
        }
        lbm = null;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.menu_reboot_vehicle: {
                SlideToUnlockDialog rebootDialog = SlideToUnlockDialog.newInstance(
                        getString(R.string.reboot_vehicle_unlock), new Runnable() {
                            @Override
                            public void run() {
                                rebootVehicle();
                            }
                        });
                rebootDialog.show(getSupportFragmentManager(), "Slide to reboot the flight controller");
                return true;
            }
    ...
    private void rebootVehicle() {
        final Drone drone = dpApp.getDrone();
        if (drone == null || !drone.isConnected()) {
            Toast.makeText(getApplicationContext(), R.string.reboot_vehicle_no_drone, Toast.LENGTH_LONG).show();
            return;
        }
