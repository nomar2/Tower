/*
 * com.MAVLink.MAVLinkPacket from the bundled MAVLink Java library
 * (dronekit-android 3.0.2, libs/Mavlink.jar), patched for MAVLink 2.
 *
 * The big unpack() message factory is the stock 2016 dialect, left as-is. The
 * MAVLink 2 changes are the extra fields/constants, the (int, boolean)
 * constructor, generateCRC(), encodePacket() / encodePacketMavlink2() and a
 * bounds-guarded unpack path. See README.md in this folder.
 */
package com.MAVLink;
...
implements Serializable {
    ...
    /* Set to true by the parser once a valid MAVLink 2 frame has been decoded
     * from the peer, so outgoing packets are also framed as v2 (auto-negotiation). */
    public static volatile boolean sendMavlink2 = false;
    ...
    public MAVLinkPacket(int payloadLength, boolean mavlink2) {
        this.len = payloadLength;
        this.isMavlink2 = mavlink2;
        // For v2, trailing zero bytes of the payload are truncated on the wire.
        // Allocate the full buffer (zero-filled) so message unpackers can read
        // past the truncated length without an IndexOutOfBoundsException.
        this.payload = new MAVLinkPayload(mavlink2 ? MAX_PAYLOAD_SIZE : payloadLength);
    }
    ...
    public void generateCRC() {
        ...
        int payloadSize = this.payload.size();
        if (this.isMavlink2) {
            this.crc.update_checksum(payloadSize);
            this.crc.update_checksum(this.incompatFlags);
            this.crc.update_checksum(this.compatFlags);
            this.crc.update_checksum(this.seq);
            this.crc.update_checksum(this.sysid);
            this.crc.update_checksum(this.compid);
            this.crc.update_checksum(this.msgid & 0xFF);
            this.crc.update_checksum((this.msgid >> 8) & 0xFF);
            this.crc.update_checksum((this.msgid >> 16) & 0xFF);
        } else {
        ...
        try {
            ...
            // Unknown message id (v2 message outside this dialect's CRC table).
            // Leave the checksum unfinished so the frame fails validation and is
            // dropped, which is the desired behaviour for messages we cannot decode.
        }
    }

    public byte[] encodePacket() {
        if (this.isMavlink2 || sendMavlink2) {
            return encodePacketMavlink2();
        }
        byte[] buffer = new byte[6 + this.len + 2];
        int i = 0;
        buffer[i++] = -2;
        buffer[i++] = (byte)this.len;
        buffer[i++] = (byte)this.seq;
        buffer[i++] = (byte)this.sysid;
        buffer[i++] = (byte)this.compid;
        buffer[i++] = (byte)this.msgid;
        int payloadSize = this.payload.size();
        for (int j = 0; j < payloadSize; ++j) {
            buffer[i++] = this.payload.payload.get(j);
        }
        this.generateCRC();
        buffer[i++] = (byte)this.crc.getLSB();
        buffer[i++] = (byte)this.crc.getMSB();
        return buffer;
    }

    private byte[] encodePacketMavlink2() {
        // We never emit the optional payload truncation or a signature, so the
        // frame is 10 header bytes + payload + 2 CRC bytes.
        this.isMavlink2 = true;
        this.incompatFlags = 0;
        this.compatFlags = 0;
        int payloadSize = this.payload.size();
        byte[] buffer = new byte[10 + payloadSize + 2];
        int i = 0;
        buffer[i++] = (byte)MAVLINK_STX_MAVLINK2;
        buffer[i++] = (byte)payloadSize;
        buffer[i++] = (byte)this.incompatFlags;
        buffer[i++] = (byte)this.compatFlags;
        buffer[i++] = (byte)this.seq;
        buffer[i++] = (byte)this.sysid;
        buffer[i++] = (byte)this.compid;
        buffer[i++] = (byte)(this.msgid & 0xFF);
        buffer[i++] = (byte)((this.msgid >> 8) & 0xFF);
        buffer[i++] = (byte)((this.msgid >> 16) & 0xFF);
        for (int j = 0; j < payloadSize; ++j) {
            buffer[i++] = this.payload.payload.get(j);
        }
        this.generateCRC();
